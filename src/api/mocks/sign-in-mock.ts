...
export const signInMock = http.post<never, ISignInBody>(
  ...
  async ({ request }) => {
    ...
    if (email === 'manoelprado.aecjr@gmail.com' && password === 'senha123') {
      const fakeJwt =
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsImlhdCI6MTczNDA2ODM1MSwiZXhwIjoxNzM0MDcxOTUxfQ.NkoEZaNO_LHtMGhldbWcOKsQHiyNU7AwypPtEF2Tuk4'

      return new HttpResponse(JSON.stringify(fakeJwt), {
        status: 200,
      })
    }
